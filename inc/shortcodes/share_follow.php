...
function flatsome_share($atts, $content = null) {
	...
	<div class="social-icons share-icons share-row relative icon-style-<?php echo $style; ?> <?php echo $align; ?>" <?php echo $scale;?>>
		  ...
		  <a href="whatsapp://send?text=<?php echo $whatsapp_text; ?>" data-action="share/whatsapp/share" class="<?php echo $classes;?> whatsapp show-for-medium" title="<?php _e('Podijeli putem WhatsApp aplikacije','flatsome'); ?>"><i class="icon-phone"></i></a>
		  <?php } if(in_array('facebook', $share)){ ?>
		  <a href="//www.facebook.com/sharer.php?u=<?php echo $permalink; ?>" data-label="Facebook" onclick="window.open(this.href,this.title,'width=500,height=500,top=300px,left=300px');  return false;" rel="nofollow" target="_blank" class="<?php echo $classes;?> facebook" title="<?php _e('Podijeli na Facebooku','flatsome'); ?>"><?php echo get_flatsome_icon('icon-facebook'); ?></a>
		  <?php } if(in_array('twitter', $share)){ ?>
          <a href="//twitter.com/share?url=<?php echo $permalink; ?>" onclick="window.open(this.href,this.title,'width=500,height=500,top=300px,left=300px');  return false;" rel="nofollow" target="_blank" class="<?php echo $classes;?> twitter" title="<?php _e('Podijeli na Twitteru','flatsome'); ?>"><?php echo get_flatsome_icon('icon-twitter'); ?></a>
          <?php } if(in_array('email', $share)){ ?>
          <a href="mailto:enteryour@addresshere.com?subject=<?php echo $post_title; ?>&amp;body=Check%20this%20out:%20<?php echo $permalink; ?>" rel="nofollow" class="<?php echo $classes;?> email" title="<?php _e('Podijeli putem e-maila','flatsome'); ?>"><?php echo get_flatsome_icon('icon-envelop'); ?></a>
          ...
          <a href="//plus.google.com/share?url=<?php echo $permalink; ?>" target="_blank" class="<?php echo $classes;?> google-plus" onclick="window.open(this.href,this.title,'width=500,height=500,top=300px,left=300px');  return false;" rel="nofollow" title="<?php _e('Podijeli na Google+','flatsome'); ?>"><?php echo get_flatsome_icon('icon-google-plus'); ?></a>
          ...
          <a href="//www.linkedin.com/shareArticle?mini=true&url=<?php echo $permalink; ?>&title=<?php echo $post_title; ?>" onclick="window.open(this.href,this.title,'width=500,height=500,top=300px,left=300px');  return false;"  rel="nofollow" target="_blank" class="<?php echo $classes;?> linkedin" title="<?php _e('Podijeli na LinkedInu','flatsome'); ?>"><?php echo get_flatsome_icon('icon-linkedin'); ?></a>
...
function flatsome_follow($atts, $content = null) {
    ...
    <div class="social-icons follow-icons <?php echo $align; ?>" <?php echo $scale;?>>
		...
		<?php }?>
    	<?php if($facebook){?>
    	<a href="<?php echo $facebook; ?>" target="_blank" data-label="Facebook"  rel="nofollow" class="<?php echo $style; ?> facebook tooltip" title="<?php _e('Pronađite nas na Facebooku','flatsome') ?>"><?php echo get_flatsome_icon('icon-facebook'); ?>
    	</a>
		<?php }?>
		<?php if($instagram){?>
		    <a href="<?php echo $instagram; ?>" target="_blank" rel="nofollow" data-label="Instagram" class="<?php echo $style; ?>  instagram tooltip" title="<?php _e('Pratite nas i na Instagramu','flatsome')?>"><?php echo get_flatsome_icon('icon-instagram'); ?>
		   </a>
		<?php }?>
		<?php if($snapchat){?>
		...
		<?php }?>
		<?php if($twitter){?>
	       <a href="<?php echo $twitter; ?>" target="_blank"  data-label="Twitter"  rel="nofollow" class="<?php echo $style; ?>  twitter tooltip" title="<?php _e('Pronađite nas na Twitteru','flatsome') ?>"><?php echo get_flatsome_icon('icon-twitter'); ?>
	       </a>
		<?php }?>
		<?php if($email){?>
		     <a href="mailto:<?php echo $email; ?>" target="_blank"  data-label="E-mail"  rel="nofollow" class="<?php echo $style; ?>  email tooltip" title="<?php _e('Pošaljite nam e-mail','flatsome') ?>"><?php echo get_flatsome_icon('icon-envelop'); ?>
			</a>
		<?php }?>
		<?php if($pinterest){?>
		...
		<?php }?>
		<?php if($youtube){?>
		       <a href="<?php echo $youtube; ?>" target="_blank" rel="nofollow" data-label="YouTube" class="<?php echo $style; ?>  youtube tooltip" title="<?php _e('Pretplatite se na naš YouTube kanal','flatsome') ?>"><?php echo get_flatsome_icon('icon-youtube'); ?>
		       </a>
		<?php }?>
		<?php if($flickr){?>
